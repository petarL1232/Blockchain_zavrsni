...
public final class ConsensusRules {
    ...
    public static boolean isRegularTransactionValid(Transactions tx, Map<String, PublicWallet> walletRegistry) {
        ...
        if (!walletRegistry.containsKey(tx.getSender()) || !walletRegistry.containsKey(tx.getReceiver())) {
            return false;
        }
    ...
    public static boolean isBlockHashValid(Block block) {
        return block != null && isHeaderHashValid(block.index, block.previousHash, block.timestamp,
                block.getMerkleRoot(), block.nonce, block.hash, block.getDifficulty());
    }

    public static boolean isHeaderHashValid(int index, String previousHash, long timestamp, String merkleRoot,
            long nonce, String hash, int difficulty) {
        return hash != null
                && hash.equals(Block.calculateHeaderHash(index, previousHash, timestamp, merkleRoot, nonce, difficulty));
    }
